...
exports.getAppointments = async (req, res, next) => {
    ...
    if(req.user.role !== 'admin'){
        ...
        if(req.params.hospitalId){
            console.log(String(req.params.hospitalId))

            /////////////////////////////////code fixed///////////////////////////////
            const id = mongoose.Types.ObjectId(req.params.hospitalId);

            query = Appointment.find({hospital: req.params.hospitalId}).populate({
                path:'hospital',
                select:'name province tel',
            })
            // query = Appointment.findById("62320c480c74bfbf4f65c30b");
        } else {
...
exports.deleteAppointment = async (req, res, next) => {
    try {
        ...
        if(!appointment){
            return res.status(404).json({success: true, message: `No appt with id ${req.params.it}`});
        }
